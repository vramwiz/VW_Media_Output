...
- 簡易設定ダイアログ
- INI による前回設定の保存
- 通常 MP4 の 0 / 90 / 180 / 270 度回転メタデータ出力テスト
...
- Output mode
  - `Normal MP4`
  - `Alpha MOV / ProRes 4444`
- Encoder
  - `GPU / H.264 Intel QSV`
  - `CPU / H.264 libx264`
  - `GPU / H.264 NVIDIA NVENC`
  - `GPU / H.264 AMD AMF`
- Video quality
  - `High quality`
  - `Standard`
...
- Audio
  ...
  - `None`
- Rotation metadata
  - 通常 MP4 のみ、MP4 の display matrix / rotate metadata で 0 / 90 / 180 / 270 度の時計回り回転を指定するテスト用設定

通常出力は `Standard`、試し出力は `Fast`、画質を優先したい場合は `High quality` を選びます。

## 回転出力テスト

`Rotation metadata` で `90 deg clockwise` / `180 deg` / `270 deg clockwise` を選ぶと、通常 MP4 出力で映像ストリームに display matrix / rotate metadata を付与します。

- エンコードされる画素データと出力動画の幅/高さは入れ替えません。
- display matrix / rotate metadata を付与する機能です。
- Alpha MOV / ProRes 4444 では透過保持経路を優先するため、回転 metadata は付与しません。
- エンコードプレビューは metadata 適用後の見え方に合わせて 90 / 180 / 270 度回転表示します。

この機能は、縦長ショート動画を Media Player と VideoMiner で比較するときの切り分け用です。画素データは元の向きのまま、回転 metadata だけを付けた素材で差が出る場合は、プレイヤー側の回転メタデータ解釈が主因と考えられます。
...
RotateOutput90Degrees=False
ShowCheckLogAfterEncode=False
```
...
- `Plugin_Output\FFmpegOutputEncoder.pas`
  - 映像/音声取得、変換、エンコード、mux の中心処理。通常 MP4 の回転 metadata 出力もここで行う。
...
- `Plugin_Output\FFmpegOutputPreview.pas`
  - エンコード中のプレビュー画面と暗いフレーム検査ログ。回転出力テスト時はプレビューも同じ向きに変換する。
